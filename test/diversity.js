...
module.exports = {
  ...
  'solve genetic diversity problem': function(beforeExit, assert) {
    ...
    function mutationFunction(phenotype) {
      const result = cloneJSON(phenotype);
    ...
    function crossoverFunction(a, b) {
		    const x = {number: [a.number[0], b.number[1]]};
		    const y = {number: [b.number[0], a.number[1]]};
		    return Math.random() > 0.5 ? [x, y] : [y, x];
    }

    function score(phenotype) {
      let score = Math.sin( phenotype.number[0] * Math.PI ) *
    ...
    function doesABeatBFunction(a, b) {
      const doesABeatB = ( distance(a, b) < 2 ) &&
    ...
    for ( let tries = 0; tries < 20 && ( best.number[0] < 3 || best.number[1] < 3 ); tries++ ) {
